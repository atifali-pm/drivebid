...
export const api = {
  ...
  hideRide: (rideId: number) =>
    request<{ status: string }>(`/rides/${rideId}/hide`, { method: "POST" }),

  unhideRide: (rideId: number) =>
    request<{ status: string }>(`/rides/${rideId}/hide`, { method: "DELETE" }),

  listHiddenRides: () => request<Ride[]>("/rides/hidden"),
